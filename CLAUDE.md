...
Both halves are meant to share the same data by reading/writing the same **Supabase** project (Postgres + REST API). Menu images are intended to be stored in **Cloudflare** (not Supabase Storage). Supabase Auth (login + persisted session, see `models/` below and `views/sesion_view.py`) is wired up as of Phase 2.0. `menu_view.py` reads real rows from `public.platillos` via `models/platillo_dao.py` as of Phase 2.1-2.5 (loading/empty/error states, async fetch), and as of Phase 2.6 the panel can create/edit/delete/hide platillos too — see `models/`/Architecture below. Still not wired: Cloudflare image upload and the AI agent.
...
The menu table exists: **`public.platillos`** (lowercase — `.from_("platillos")` in code) with `id, nombre, descripcion, categoria, precio, visible, image_url, orden, created_at`. `categoria` carries a CHECK constraint limiting it to `Platillos` / `Bebidas` / `Postres`. RLS is on: `anon` may only `SELECT` rows where `visible = true`; `authenticated` has full read/write. It is seeded with the 10 placeholder dishes that `menu_view.py` used to hardcode. **Images are Cloudflare's job — do not enable or use Supabase Storage.**
...
### `home_view.py` — most of it is still placeholder data

Almost every number on the Inicio screen is hardcoded and **not** backed by anything. Don't treat these as real, and don't invent data sources for them:

- Fake, and staying fake until Phase 7 (there is no visits table, no sales table, no content table yet): `"347 veces"` / `"+12% vs ayer"`, `"Tacos al Pastor" / "48 órdenes"`, `"12 Posts"`, the whole black `SUGERENCIA DE HOY` card, and the greeting `"Buenos días, Ary"` (fixed string — it does not change with the time of day).
- Genuinely wired: only the date line at the top (`datetime.now()`) and the navigation `on_click`s.
- Being wired in Phase 2.7 (the three cards that depend on the menu): the `PRODUCTOS EN VENTA` stat card, the `Lo último de tu menú` card, and the `"Agregar un producto nuevo"` shortcut.

`_crear_tarjeta_stat(titulo, valor, subtitulo, icono)` and `_crear_boton_atajo(titulo, subtitulo, icono, color_icono, ruta)` are the two builders; `HomeView` currently builds everything synchronously in `__init__` with no async fetch and no loading state, unlike `MenuView`. Wiring any card to Supabase means adding the `page.run_task` + `asyncio.to_thread` pattern that `menu_view.py`/`sesion_view.py` already use.

**Schema note for `Lo último de tu menú`**: `public.platillos` has `created_at` but **no `updated_at`**, so as things stand you can detect newly *added* platillos but not *edited* ones. Making that card show real "cambios" (which is what its subtitle promises) requires a schema change — an `updated_at` column plus a trigger to bump it on `UPDATE`. That's a decision to confirm with the owner, not to assume.
...
- `controllers/main_controller.py` — `MainController` acts as the app's router/state holder (referred to as `router` throughout the view/component code). It owns:
  ...
  - `cambiar_vista(vista: str)` — the *only* navigation mechanism: instantiates the matching view class, sets it as the switcher's content, and rebuilds the `Sidebar` so its active-button highlighting stays in sync. There is no separate router/state-management library.
