...
Both halves are meant to share the same data by reading/writing the same **Supabase** project (Postgres + REST API). Menu images are stored in **Cloudflare R2** (not Supabase Storage). Supabase Auth (login + persisted session, see `models/` below and `views/sesion_view.py`) is wired up as of Phase 2.0. `menu_view.py` reads real rows from `public.platillos` via `models/platillo_dao.py` as of Phase 2.1-2.5 (loading/empty/error states, async fetch), and as of Phase 2.6 the panel can create/edit/delete/hide platillos too — see `models/`/Architecture below. As of Phase 2.7, `home_view.py`'s menu-dependent cards (the platillo/category count, the recent-changes list, and the "add new dish" shortcut) are wired to real data too — see the dedicated `home_view.py` section under Design rules below. As of Phase 3, `DialogoPlatillo` can pick a local photo (via Tkinter, not Flet's `FilePicker` — an explicit owner preference), preview it, and upload it to R2 through the `taku-monky-uploads` Worker on save — see `models/cloudflare_storage.py` and the dedicated Phase 3 sections below. Still not wired: the AI agent.
